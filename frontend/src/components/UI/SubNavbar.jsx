...
const SubNavbar = () => {
  ...
  const handleSearch = () => {
    ...
    navigate(`${EcomRoutes.SHOP_SEARCH}/${search}`);
    setSearch("");
  };
  ...
  return (
    <>
      <nav className="bg-[hsl(var(--brand-primary)/0.92)] backdrop-blur-md py-3 z-50 relative">
        <div className="max-w-5xl mx-auto px-4 flex flex-col lg:flex-row items-center gap-4 lg:gap-6">
          <div className="w-full lg:w-2/3 relative">
            <div className="flex items-center bg-white rounded-full overflow-visible shadow-md">
                  ...
                  <div className="absolute top-full left-0 mt-2 bg-white shadow-lg rounded-lg w-52 z-50 border">
                    {categories.map((cat) => (
                      <div
                        key={cat.slug}
                        onClick={() => {
                          navigate(`${EcomRoutes.SHOP_CATEGORY}/${cat.slug}`);
                          setShowDropdown(false);
                        }}
                ...
                {filteredProducts.length > 0 ? (
                  filteredProducts.slice(0, 6).map((item) => (
                    <div
                      key={item.slug}
                      onClick={() => {
                        navigate(`${EcomRoutes.SHOP_PRODUCT}/${item.slug}`);
                        setSearch("");
                      }}
